...
class Simulation:
    ...
    def step(self):
        ...
        """ 
        step_time = self.heap.min().time
        
        # Need to apply all the transitions at the same time before propagating.
        transitions = []
        while len(self.heap) > 0 and self.heap.min().time == step_time:
          transition = self.heap.pop()
        ...
        for transition in transitions:
          for gate in transition.gate.out_gates:
            ...
            self.heap.append(Transition(gate, output, time))
    ...
    def run(self):
        ...
        for in_transition in sorted(self.in_transitions):
            self.heap.append(Transition(in_transition[3], in_transition[2],
                                         in_transition[0]))
